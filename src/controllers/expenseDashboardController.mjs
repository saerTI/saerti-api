// src/controllers/expenseDashboardController.mjs
// Controlador para endpoints de dashboard de egresos

import * as expenseDashboardModel from '../models/expenseDashboardModel.mjs';
 ...
 */
export async function getExpenseDashboardSummary(req, res) {
  try {
    ...
    console.log('[expenseDashboardController] Filtros recibidos:', filters);
    ...
    const [
      ...
      expenseDashboardModel.getDashboardSummary(organization_id, filters),
      expenseDashboardModel.getDistributionByType(organization_id, filters),
      expenseDashboardModel.getHierarchicalDistribution(organization_id, filters),
      expenseDashboardModel.getDistributionByStatus(organization_id, filters),
      expenseDashboardModel.getTopExpenseTypes(organization_id, filters, 5),
      expenseDashboardModel.getRecentExpenses(organization_id, filters, 10)
    ]);
    ...
    if (date_from && date_to) {
      ...
      const comparison = await expenseDashboardModel.getPeriodComparison(
        organization_id,
        date_from,
        date_to,
    ...
    res.json({
      ...
      data: {
        ...
        trend_direction,
        top_expense_types: topTypes.map(t => ({
          type_id: t.type_id,
          type_name: t.type_name,
          type_color: t.type_color,
          total_amount: parseFloat(t.total_amount) || 0,
    ...
    console.error('Error en getExpenseDashboardSummary:', error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/expenses/dashboard/by-type
 * Retorna distribución por tipo de egreso
 */
export async function getExpensesByType(req, res) {
  try {
    const { organization_id } = req.user;
    ...
    const data = await expenseDashboardModel.getDistributionByType(organization_id, filters);
...
/**
 * GET /api/expenses/dashboard/by-category
...
export async function getExpensesByCategory(req, res) {
  try {
    ...
    const data = await expenseDashboardModel.getHierarchicalDistribution(organization_id, filters);
...
/**
 * GET /api/expenses/dashboard/cash-flow
 * Retorna flujo de caja por período
 */
export async function getExpenseCashFlow(req, res) {
  try {
    const { organization_id } = req.user;
    ...
    const data = await expenseDashboardModel.getCashFlowByPeriod(organization_id, period, filters);
    ...
    console.error('Error en getExpenseCashFlow:', error);
    res.status(500).json({
      success: false,
 ...
 */
export async function getExpenseTrends(req, res) {
  try {
    const { organization_id } = req.user;
    ...
    console.error('Error en getExpenseTrends:', error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/expenses/dashboard/category-by-period
 * Retorna distribución por categoría y período
 */
export async function getExpensesCategoryByPeriod(req, res) {
  try {
    const { organization_id } = req.user;
    const { date_from, date_to, period = 'month', cost_center_id } = req.query;
    ...
    const data = await expenseDashboardModel.getCategoryByPeriod(organization_id, period, filters);
    ...
    console.error('Error en getExpensesCategoryByPeriod:', error);
    res.status(500).json({
      success: false,
