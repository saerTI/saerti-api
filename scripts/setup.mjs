...
// 🆕 Crear tabla de previsionales (TEMPORAL sin foreign keys)
  ...
  try {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS previsionales (
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        -- TODO: Agregar cuando se implemente funcionalidad de proyectos:
        -- FOREIGN KEY (project_id) REFERENCES construction_projects(id) ON DELETE SET NULL
      )
    `);
    console.log('✅ Tabla previsionales creada (sin foreign key constraint - temporal)');
  } catch (error) {
...
// 🆕 Crear tabla de remuneraciones (TEMPORAL sin foreign keys)
async function createRemuneracionTable() {
  try {
    ...
    `);
    console.log('✅ Tabla remuneraciones creada (sin foreign key constraint - temporal)');
  } catch (error) {
...
async function setup() {
  ...
  try {
    ...
    // 🆕 Crear tablas de remuneraciones y previsionales (TEMPORAL sin foreign keys)
    ...
    console.log('⚠️  IMPORTANTE - Configuración temporal:');
    console.log('   📝 Tablas previsionales y remuneraciones creadas sin foreign key constraints');
    console.log('   📝 project_id permite NULL en ambas tablas');
    console.log('   📝 TODO: Restaurar constraints cuando se implemente funcionalidad de proyectos');
